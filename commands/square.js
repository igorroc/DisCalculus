...
module.exports.run = async (bot, message, args) => {
    ...
    log.send(`\\▶ [LOGS] ⇥ Usuário \` ${message.author.username} \` usou o comando \` Square \` no server \` ${message.guild.name} \`\n`)
    ...
    if(valor.search(/[^0-9]/gm) > -1){
        if(valor.match(/[.]/g)||[].length > 1){
            ...
            console.log(`↳ ⚠️ Usuário '${message.author.username}' digitou caractere: '${valor}'`)
            log.send(`↳ \\⚠️  Usuário \` ${message.author.username} \` digitou caractere: \`${valor}\``)
